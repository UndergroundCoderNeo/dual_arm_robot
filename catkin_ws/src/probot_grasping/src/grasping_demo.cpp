...
#include "probot_grasping/grasping_demo.h"//在probot_grasping 这个路径下的宏文件
//GraspingDemo是构造函数，用于初始化GraspingDemo这个类
//构造函数初始化列表：它的功能和我们写在函数体里的赋初值是一样的
GraspingDemo::GraspingDemo(ros::NodeHandle n_, float pregrasp_x, float pregrasp_y, float pregrasp_z, float length, float breadth) :
    it_(n_), //it为image topic
    armgroup("manipulator"), 
    grippergroup("gripper"), 
    vMng_(length, breadth)//赋值初始化
{
  this->nh_ = n_;//给对象赋值
